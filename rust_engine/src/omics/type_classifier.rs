...
mod tests {
    ...
    fn test_rna_seq_classified_as_transcriptomic() {
        let c = classify_omic_type("RNA-Seq of human heart tissue", "gene expression profiling");
        assert_eq!(c.omic_type, "transcriptomic");
    }

    #[test]
    fn test_chip_seq_classified_as_epigenomic() {
        let c = classify_omic_type("ChIP-seq of H3K27ac marks", "histone modification study");
        assert_eq!(c.omic_type, "epigenomic");
    }

    #[test]
    fn test_microbiome_classified_correctly() {
        let c = classify_omic_type("16S rRNA amplicon sequencing", "gut microbiome diversity");
        assert_eq!(c.omic_type, "microbiome");
    }

    #[test]
    fn test_gwas_classified_as_genomic() {
        let c = classify_omic_type("Genome-wide association study of BMI", "GWAS SNP array");
        assert_eq!(c.omic_type, "genomic");
    }

    #[test]
    fn test_fallback_to_other() {
        let c = classify_omic_type("Unknown dataset", "no keywords here");
        assert_eq!(c.omic_type, "other");
    }

    #[test]
    fn test_epigenomic_wins_over_genomic() {
        // 'methylation' is epigenomic; 'genome' should NOT override to 'genomic'
        let c = classify_omic_type("Whole genome bisulfite sequencing", "methylation patterns");
        assert_eq!(c.omic_type, "epigenomic");
    }
}
